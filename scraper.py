...
def scrape_race_data(session, jcd, rno, date_str):
    ...
    # 直前情報と出走表の両方を取得（締切時刻取得の確実性を上げるため）
    ...
    # 直前情報と出走表のテキスト両方から探す
    sources = []
    if soup_before: sources.append(clean_text(soup_before.text))
    if soup_list: sources.append(clean_text(soup_list.text))
    
    for text in sources:
        # パターン1: "締切予定 10:30" や "締切10:30" (スペース有無問わず)
        # パターン2: "予定 10:30"
        match = re.search(r"(?:締切|予定).*?(\d{1,2}:\d{2})", text)
        if match:
            t_str = match.group(1).zfill(5) # 08:30
            # 明らかにおかしい時間(00:00とか)でなければ採用
            if "00:00" <= t_str <= "23:59":
                row['deadline_time'] = t_str
                break
