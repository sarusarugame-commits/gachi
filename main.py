...
def process_race(jcd, rno, today):
    ...
    # ログ: 取得開始
    log(f"🔍 [SCAN] {PLACE_NAMES[jcd]}{rno}R ({jcd}-{rno}) データ取得試行...")

    try:
        raw = scrape_race_data(sess, jcd, rno, today)
    except Exception as e:
        log(f"❌ [ERROR] {PLACE_NAMES[jcd]}{rno}R スクレイピングエラー: {e}")
        return

    if raw:
        # ログ: 取得できた生データを表示（ここが見たかったはず）
        log(f"✅ [OK] {PLACE_NAMES[jcd]}{rno}R -> 締切:{raw.get('deadline_time')} | 1号艇勝率:{raw.get('wr1')} | 1号艇モーター:{raw.get('mo1')}")
    else:
        log(f"💨 [SKIP] {PLACE_NAMES[jcd]}{rno}R: データなし（開催なしか、まだ情報未公開）")
        return

    # 安全装置解除: データがどうあれ予測に突っ込む
    try:
        preds = predict_race(raw)
    except Exception as e:
        log(f"❌ [ERROR] {PLACE_NAMES[jcd]}{rno}R 予測プロセスエラー: {e}")
        return

    if not preds: 
        # 予測結果が空（＝投資対象外）の場合もログを出す
        # log(f"💤 [PASS] {PLACE_NAMES[jcd]}{rno}R: 推奨買い目なし")
        return
    ...
    for p in preds:
        ...
        if not exists:
            log(f"🔥 [HIT] {PLACE_NAMES[jcd]}{rno}R 激熱発見！ -> {p['combo']}")
...
def main():
    log("🚀 最強AI Bot (強制実行・ログ全開モード) 起動")
    init_db()
    threading.Thread(target=report_worker, daemon=True).start()
    
    while True:
        today = datetime.datetime.now(JST).strftime('%Y%m%d')
        log(f"\n⚡ Scan Loop Start: {datetime.datetime.now(JST).strftime('%H:%M:%S')}")
